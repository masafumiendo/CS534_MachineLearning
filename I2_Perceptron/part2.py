...
class AvPerceptron:
    ...
    def __init__(self, x_train, y_train, x_valid, y_valid, iters):
        ...
        self.av_weight = np.zeros(x_train.shape[1])
        self.av_weight_list = []
    ...
    def train(self):
        ...
        for _iter in range(self.iters):
            ...
            for i in range(self.num_sample_train):
                ...
                self.av_weight = (self.ex_count * self.av_weight + self.weight)/(self.ex_count + 1)
                self.ex_count += 1
            ...
            av_weight_iter = self.av_weight

            self.av_weight_list.append(av_weight_iter)
    ...
    def predict(self, x):
        ...
        best_av_weight = self.av_weight_list[max_acc_index]

        y_test = np.zeros(x.shape[0])

        for i in range(x.shape[0]):
            # Compute weight dot x to obtain y_hat
            z = np.dot(best_av_weight, x[i, :])
            y_hat = np.sign(z)
    ...
    def __error_graph(self, train, valid, figname, option):
        ...
        if option == "both":
            ...
            plt.savefig('fig/part2/training_' + figname)
            plt.close(fig)
            ...
            plt.savefig('fig/part2/validation_' + figname)
            plt.close(fig)
