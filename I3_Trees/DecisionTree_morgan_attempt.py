...
class DecisionTree:
    ...
    def make_tree(self, df, depth=0, sample_weight=None, sample_m_features=False, m_features=5):
        ...
        if self.__check_pure(df) == True or depth == self.max_depth:
            ...
            if sample_m_features == True:
                df = self.__get_m_features(self.global_df, m_features)
            ...
            ans_0 = self.make_tree(df_0, depth, sample_m_features=sample_m_features, m_features=m_features)
            ans_1 = self.make_tree(df_1, depth, sample_m_features=sample_m_features, m_features=m_features)
    ...
    def __get_m_features(self, df, m_features):
        # sample m features for tree
        features = list(df.drop("Class", axis=1).columns)
        new_features = sample(features, m_features)
